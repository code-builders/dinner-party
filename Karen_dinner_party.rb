...
["Karen", "Wolfie", "Erin", "Tim", "Thom", "Stephen", "Ken", "James"]}
...
party_elements[:people].each do |person|
  puts "#{person} will have the #{party_elements[:food].sample} to eat with #{party_elements[:food].sample} on the side. And to drink, #{party_elements[:beverages].sample}! Yum!"
end
